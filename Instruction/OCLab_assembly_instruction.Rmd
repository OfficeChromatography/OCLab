...
<!-- ![Y axis. \label{figureYaxis}](figure/Y axis.png){width=33%} -->
...
Assemble the ```X_motor```, ```X_end``` and ```X_moving``` parts as for the Y axis. Note that the ```X_moving``` and ```X_end_stop_holder``` are taken from the other designs, the STL files are in the STL folder separatly but also in the main STL file. 
...
![LED wiring. \label{figureLEDWiring}](figure/wiring_LED.jpg){width=200%}
