...
Some knowledge are mandatory to build this device. The following links should give an overview. Over this document, it is assumed a significant amount of time had been spend on those website nd that their content is known.
...
![Y axis SCAD. \label{figureYaxis}](figure/Y axis.png){width=33%}
![Y axis before.](figure/yaxis_before.jpg){width=33%}
![Y axis after.](figure/yaxis_after.jpg){width=33%}
...
![X axis SCAD. \label{figureXaxis}](figure/X axis.png){width=33%}
![X axis before.](figure/xaxis_before.jpg){width=33%}
![X axis after.](figure/xaxis_after.jpg){width=33%}

Assemble the ```X_motor```, ```X_end``` and ```X_moving``` parts as for the Y axis. No te that the ```X_moving``` and ```X_end_stop_holder``` are taken from the other designs, the STL files are in the STL folder. 

Note that the ```X_moving``` part is taken from the prusa i3 design, it can be found in stl format in the SCAD and STL folder under the name ```i3rework_Xcarriage.stl```.
...
![Electronique SCAD. \label{figureElectronique}](figure/elec.png){width=50%}
![Electronique.](figure/elec_wired.jpg){width=50%}
...
![LED wiring. \label{figureLEDWiring}](figure/wiring_LED.jpg){width=80%}
