...
Some knowledge are mandatory to build this device. The following ressources should give an overview.
...
SCAD files can be found in thte SCAD fodler, normally, only the file OCLab.scad is of interest. Each different module can be observed separatly. To see the full apparatus, use the ```full_view``` module, the argument, ```printed``` 

### STL folder

The STL files in this folder can contain either a specific part of the system (Y axis, X axsis etc...) or the full view with all the part. They are designed to be printed on a Prusa i3 MK2 with the _0.35mm fast_ setting. Normally, no supports are needed. Be sure to rotate the part so they print correctly.
...
```{r}
data = read_excel("../BOM_OCLab.xlsx")
kable(data,caption = "Bill of material")
```
...
* M5 nuts
* 20x20 profiles
* 20x20 cube connectors
* 20x20 angle connectors

Before assembling the frame, set the nuts to have one nut for each M5 screw shown in Figure \ref{figureFrame}. There is XXX in total, if needed, use the SCAD file to ge a 3D view: the module ```full_view``` with ```type = frame```.

![Frame assembly. \label{figureFrame}](figure/FigureFrame.png){width=50%}
...
![Y axis. \label{figureYaxis}](figure/Figure Y axis.png){width=50%}
...
* 1x nema14
* 1x gt2 pulley
* printed parts: ```X_motor```, ```X_end```, ```X_moving```, ```HP_C6602_holder_holder```
* HP C6602 holder
* 4x M4x20 (may be not 20, need check)
* 4x M4 nut
* 1x M3x30
* 4x M3x16
* 3x M2*15 (may be not 15, need check)
* 3x M3 nut

![X axis. \label{figureXaxis}](figure/Figure X axis.png){width=50%}
...
The X axis must be position as low as possible while avoiding colision with the plate hodler. The Y axsi will be positioned later when the cartridge will be able to fire.
...
![Electronique. \label{figureElectronique}](figure/Electronique.png){width=50%}
...
Follow the instruction on the github repository of [OC_manager](https://github.com/DimitriF/OC_manager). Steps could be missing and feedback are more than welcome.
